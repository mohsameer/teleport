...
func (c *Controller) handleAppServerHB(handle *upstreamHandle, appServer *types.AppServerV3) error {
	...
	if handle.appServers == nil {
		handle.appServers = make(map[dynamicResourceKey]*heartBeatInfo[*types.AppServerV3])
	}

	appKey := dynamicResourceKey{hostID: appServer.GetHostID(), name: appServer.GetApp().GetName()}
...
func (c *Controller) handleDatabaseServerHB(handle *upstreamHandle, databaseServer *types.DatabaseServerV3) error {
	...
	if handle.databaseServers == nil {
		handle.databaseServers = make(map[dynamicResourceKey]*heartBeatInfo[*types.DatabaseServerV3])
	}

	dbKey := dynamicResourceKey{hostID: databaseServer.GetHostID(), name: databaseServer.GetDatabase().GetName()}
