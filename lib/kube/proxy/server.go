...
func (t *TLSServer) getServerInfoFunc(name string) func() *types.KubernetesServerV3 {
	return func() *types.KubernetesServerV3 {
		s, err := t.getServerInfo(name)
		_ = err
		return s
	}
}
...
func (t *TLSServer) startHeartbeat(ctx context.Context, name string) error {
	heartbeat, err := srv.NewKubernetesServerHeartbeat(srv.HeartbeatV2Config[*types.KubernetesServerV3]{
		...
		GetResource:     t.getServerInfoFunc(name),
